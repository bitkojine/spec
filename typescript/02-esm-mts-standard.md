...
- **REQUIRED: Explicit Signaling**: All TypeScript source files MUST use either the `.mts` or `.cts` extension.
    - **.mts**: Use for all browser-compatible or modern Node.js ESM code (e.g., Webview components).
    - **.cts**: Use for code that must maintain CommonJS compatibility (e.g., VS Code Extension Host, E2E tests).
...
- **REQUIRED: Explicit Extensions**: All relative imports MUST include the explicit extension of the **compiled target**:
    ...
    - Import `baz.ts` (if any legacy files remain) as `import "./baz.js"`.
...
| Source File | Context | Resolution Mode | Target Ext | Import As |
| :--- | :--- | :--- | :--- | :--- |
| `.mts` | ESM | Node16/Next | `.mjs` | `./file.mjs` |
| `.cts` | CJS | Node16/Next | `.cjs` | `./file.cjs` |
| `.ts` | ESM (Default) | Node16/Next | `.js` | `./file.js` |
